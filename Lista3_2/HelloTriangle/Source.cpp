...
// Variáveis para armazenar as coordenadas X e Y da posição atual da geometria
float posX = 400.0f; // posição inicial X
float posY = 300.0f; // posição inicial Y
...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Modifique as transformações aqui para cada instância
		...
		model = glm::translate(model, glm::vec3(posX, posY, 0.0f)); // translada para a posição atual
...
// ou solta via GLFW
// Função de callback de teclado - é chamada sempre que uma tecla é pressionada ou solta via GLFW
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	{
		float moveSpeed = 30.0f; // velocidade de movimento

		// Move a geometria para cima (tecla W ou seta para cima)
		if (key == GLFW_KEY_W || key == GLFW_KEY_UP)
			posY += moveSpeed;

		// Move a geometria para baixo (tecla S ou seta para baixo)
		else if (key == GLFW_KEY_S || key == GLFW_KEY_DOWN)
			posY -= moveSpeed;

		// Move a geometria para a esquerda (tecla A ou seta para a esquerda)
		else if (key == GLFW_KEY_A || key == GLFW_KEY_LEFT)
			posX -= moveSpeed;

		// Move a geometria para a direita (tecla D ou seta para a direita)
