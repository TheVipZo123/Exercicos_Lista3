...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Loop para desenhar a mesma geometria três vezes com transformações diferentes
		for (int i = 0; i < 3; ++i) {
			// Modifique as transformações aqui para cada instância
			float angle = (float)glfwGetTime() + (float)i * 120.0f; // Adiciona um ângulo diferente para cada instância
			glm::mat4 model = glm::mat4(1); // matriz identidade
			model = glm::translate(model, glm::vec3( 400.0f, i/1.7 * 350.0f, 0.0f)); // translada para o centro da tela
			model = glm::rotate(model, glm::radians(angle), glm::vec3(0.0f, 0.0f, 1.0f)); // rotaciona
			model = glm::scale(model, glm::vec3( 300.0f, 300.0f, 0.0f)); // escala

			// Configure a matriz de modelo no shader
			shader.setMat4("model", glm::value_ptr(model));
